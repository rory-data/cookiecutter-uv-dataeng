...
def test_bake_project(cookies: Cookies) -> None:
    ...
    Args:
        cookies: Pytest fixture for testing cookiecutter templates.
    """
    ...
    try:
        ...
        assert result.project_path.name == "my-project", (
            f"Expected project name 'my-project', got {result.project_path.name}"
        )
    ...
    with run_within_dir(tmp_path):
        try:
            ...
            assert result.project_path.name == "example-project", (
                f"Expected project name 'example-project', got {result.project_path.name}"
            )
            assert result.project_path.is_dir(), f"Expected {result.project_path} to be a directory"
            ...
            with run_within_dir(str(result.project_path)):
                ...
                sync_result = subprocess.run([uv_path, "sync"], check=True, capture_output=True, text=True)
                assert sync_result.returncode == 0, f"UV sync failed: {sync_result.stderr}"

                # Run tests
                logger.info("Running tests in generated project")
                test_result = subprocess.run(
                    [uv_path, "run", "make", "test"], check=True, capture_output=True, text=True
                )
                assert test_result.returncode == 0, f"UV run make test failed: {test_result.stderr}"
...
def test_license_types(cookies: Cookies, tmp_path: Path, license_type: str, expected_line_count: int) -> None:
    ...
    Args:
        cookies: Pytest fixture for testing cookiecutter templates.
        tmp_path: Temporary directory fixture.
        license_type: License type to test.
        expected_line_count: Expected number of lines in the license file.
    """
